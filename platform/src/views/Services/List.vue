<template>
  <el-row v-if="can('manage services') || isSuperAdmin">
...
export default {
  ...
  computed: {
    ...mapState('services', ['servicelist', 'listLoading', 'deleteLoading']),
    ...mapState('auth', ['user']),
  ...
  methods: {
    ...mapActions('services', ['getServiceList', 'destroy', 'clearErrorState']),
    loadData() {
      this.getServiceList({
    ...
    addService() {
      ...
      this.$router.push({ name: 'service-save' })
    },
    editService(service) {
      this.clearErrorState()
      this.$router.push({ name: 'service-save', params: { id: service.id } })
    },
    ...
    async deleteService() {
      await this.destroy(this.serviceToRemove.id)
